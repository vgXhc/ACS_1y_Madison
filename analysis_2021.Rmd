...
I was especially excited/anxious about the commuting data: What did the pandemic do to people's trips to work? How much of a shift to working from home would we see? And was the data quality actually going to be good enough to draw any firm conclusions? Let's look at the data for the City of Madison.

# Mode share over time
ACS mode share data is available all the way back to 2010. In [previous posts on the Madison Bikes blog](https://www.madisonbikes.org/2020/09/biking-numbers-in-madison-not-so-great/) I had already looked at these time series data -- and frankly, it was not that exciting: By and large the percentages of different modes of getting to worked stayed the same. Biking didn't grow but neither shrunk. It seemed highly likely that the pandemic would have changed that. And it did: 
...
msn_mode_share |> 
  ...
  ylab("estimate (%)") +
  xlab(element_blank()) +
  labs(title = "Changes in commute mode, 2019 to 2021",
       subtitle ="City of Madison",
       caption = "American Community Survey 1-year estimates, Table S0801\nVisualization: @HaraldKliems") 
```
...
msn_counts |> 
  ...
  xlab(element_blank()) +
  theme(legend.position = "none") +
  labs(title = "Transit commutes have been the big loser",
       subtitle ="City of Madison commute mode share, 2010-2021, with working from home excluded",
...
p+
  #drive alone
  annotate(
    geom = "curve", x = 5.5, y = .75, xend = 6.1, yend = .561, 
    curvature = -.2, arrow = arrow(length = unit(2, "mm")), 
  ) +
    annotate(
    geom = "curve", x = 5.5, y = .75, xend = 5.95, yend = .575, 
    curvature = -.7, arrow = arrow(length = unit(2, "mm")), 
  ) +
  annotate(geom = "text", y = .755, x = 5.5, label = "Error bars don't overlap", hjust = "left") +
  #work from home
    annotate(
